...
var grungeAlbumsJSObject = JSON.parse(grungeAlbumsJSON)

function print(){
    for (var i = 0; i<grungeAlbumsJSObject.albums.length;i++){
        console.log("Album: " + grungeAlbumsJSObject.albums[i].name);
        console.log("Artist: " + grungeAlbumsJSObject.albums[i].artist);
        console.log("Units sold: " + grungeAlbumsJSObject.albums[i].unitsSold);
    }
}
...
var sharon = '{"name": "sharon","gender": "female"}';
var sharonObject = JSON.parse(sharon);
var sharonJson = JSON.stringify(sharonObject);
